...
1. 类型参数，使用尖括号括住，放在类名后面，然后在使用使用这个类的时候，再用实际的类型替换此类型参数。
2. 元组（tuple），是将一组对象直接打包存储于其中的一个单一对象，这个容器对象允许读取其中的元素，但是不允许向其中存放新的对象（数据传送对象或者信使）。一般来说，元组可以有任意长度，同时，元组中的对象可以是任意不同的类型。
3. ​
