...
11. Linked List:
    ...
    {0}. ![Screen Shot 2017-08-01 at 12.54.28 pm](https://ws1.sinaimg.cn/large/006tNc79gy1fi41if0dy1j317i0lcwge.jpg)
    {0}.  在一个linked list中添加一个新的node，其value为10，其指针或者说reference指向该链表的root node。添加完成后，该新node变成新的root node。
    {0}. ![Screen Shot 2017-08-01 at 12.55.23 pm](https://ws3.sinaimg.cn/large/006tNc79gy1fi41iizt94j317k0lcabo.jpg)
    {0}. 在尝试从linked list中删除一个元素的时候，我们先遍历链表，根据value找到对应的node，然后将这个node的前一个node的引用，指向这个node的后一个node。实际上，这个找到的node仍然存在，但是被排除在了现存的链表外。
    {0}. ![Screen Shot 2017-08-01 at 12.58.56 pm](https://ws3.sinaimg.cn/large/006tNc79gy1fi41igzfsoj31780l4q4l.jpg)

12. Python code:

    {0}. ```python
       # 定义node类，包含两个属性，data & nodeReference
       class Node(object):
